...
import { deletePostService, loadPostbySearch } from '../services/post-service'
...
function SearchBlogs({searchkey}) {
    ...
    useEffect(()=>{
        ...
        loadPostbySearch(searchkey,0,10).then((data)=>{
            console.log(data);
            setPostContent(data);
        }).catch(error=>{
            console.log(error);
            toast.error("Blog not found with this keyword!")
        })
    },searchkey)
    ...
    const changePage=(pageNumber=0, pageSize=10)=>{
        loadPostbySearch(searchkey,pageNumber,pageSize).then(data=>{
            console.log(data);
            setPostContent(data);
            window.scroll(0,0);
        }).catch(error=> {
