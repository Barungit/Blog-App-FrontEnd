...
import { deletePostService, loadPostbyCategory, loadPostbyPopularity } from '../services/post-service'
...
function Popular() {
    ...
    useEffect(()=>{
        ...
        loadPostbyPopularity(true,0,10,"view").then(data=>{
            console.log(data)
            SetPosts(data)
        }).catch(error=>{
            console.log(error);
    ...
    const changePage=(pageNumber=0, pageSize=10)=>{
        loadPostbyPopularity(true,pageNumber,pageSize,"view").then(data=>{
            console.log(data);
            SetPosts(data);
            window.scroll(0,0);
        }).catch(error=> {
