...
export const createPost = (postData) => {
  ...
  return privateAxios
    .post(
      `/user/${postData.userId}/category/${postData.categoryId}/blogs`,
      postData
    )
    .then((response) => {
...
export const loadAllPost = (visible, pageNumber, pageSize) => {
  return myAxios
    .get(
      `/blogs/status/${visible}?pageNumber=${pageNumber}&pageSize=${pageSize}`
    )
...
export const loadPostbyPopularity = (visible, pageNumber, pageSize, sortBy) => {
  return myAxios
    .get(
      `/blogs/status/${visible}?pageNumber=${pageNumber}&pageSize=${pageSize}&sortBy=${sortBy}`
    )
    .then((response) => response.data);
};
...
export const createComment = (comment, bid, uid) => {
  return privateAxios
    .post(`/blog/${bid}/comments/${uid}`, comment)
    .then((response) => response.data);
};
...
export const uploadPostImage = (bid, image) => {
  ...
  return privateAxios
    .post(`/blogs/image/upload/${bid}`, formData, {
      headers: {
...
export const loadPostbyCategory = (categoryId, pageNumber, pageSize) => {
  return privateAxios
    .get(
      `/category/${categoryId}/blogs?pageNumber=${pageNumber}&pageSize=${pageSize}`
    )
    .then((res) => res.data);
};
...
export const loadPostbyUser = (uid, pageNumber, pageSize) => {
  return privateAxios
    .get(`/user/${uid}/blogs?pageNumber=${pageNumber}&pageSize=${pageSize}`)
    .then((res) => res.data);
};
...
export const loadPostbySearch = (keyword, pageNumber, pageSize) => {
  return myAxios
    .get(
      `/blogs/search/${keyword}?pageNumber=${pageNumber}&pageSize=${pageSize}`
    )
    .then((response) => response.data);
};
