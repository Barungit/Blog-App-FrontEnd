...
export const uploadProPic = (uid, image) => {
  ...
  return myAxios
    .post(`/users/pfp/upload/${uid}`, formData, {
      headers: {
...
export const getUserDetails = (uid) => {
  return myAxios.get(`/users/${uid}`).then((response) => {
...
export const updateUserDetails = (user, uid) => {
  console.log(user);
  console.log(uid);
  return privateAxios.put(`/users/${uid}`, user).then((response) => {
    return response.data;
  });
};
...
export const changePassword = (user, uid) => {
  console.log(user);
  console.log(uid);
  return privateAxios.put(`/users/password/${uid}`, user).then((response) => {
    return response.data;
  });
};
...
export const forgotAndChangePassword = (token, pass) => {
  ...
  console.log(pass);
  return myAxios
    .put(`/users/forgotPassword/${token}/${pass}`)
    .then((response) => {
      return response.data;
    });
};
...
export const sendtoMail = (email) => {
  console.log(email);
  return myAxios.post(`/auth/fp/${email}`).then((response) => {
    return response.data;
  });
};
//delete user
export const deleteUser = (uid) => {
  return privateAxios.delete(`/users/${uid}`).then((response) => {
    return response.data;
  });
};
